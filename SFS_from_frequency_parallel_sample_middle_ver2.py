...
def runner(idx):
    ...
    if n < round(sum(Ne) / 2):
        ...
        individuals = np.arange(round(sum(Ne) / 4), round(3 * sum(Ne) / 4))
        n = sum(Ne)
    ...
    np.savetxt('SFS_L={}_N={}_s={:.6f}_m={:.6f}_tfinal={}_nsample={}_sample_middle_{}.txt'.format(L,
...
if __name__ == '__main__':
    ...
    SFS /= N_SFS
    np.savetxt('SFS_L={}_N={}_s={:.6f}_m={:.6f}_tfinal={}_nsample={}_sample_middle_avg.txt'.format(L,
